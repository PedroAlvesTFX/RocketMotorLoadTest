                             ...
                             StorageHeader &header) const
{
    ...
    uint8_t ignitionCounter = samples[0].grams >= triggerGrams ? 1 : 0;
    ...
    for (uint32_t i = 1; i < sampleCount; ++i)
    {
        ...
        {
            if (current.grams <= ((triggerGrams <= STOP_GRAMS) ? max(MIN_TRIGGER_GRAMS, triggerGrams * 0.5f) : STOP_GRAMS))
            {
