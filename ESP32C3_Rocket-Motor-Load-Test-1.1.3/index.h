...
async function startTest(){try{const r=await fetch('/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({datetime:browserDateTime(),description:document.getElementById('testDescription').value.trim().slice(0,50)})});const t=await r.text();message(t+' A atualização Web será pausada durante a gravação.')}catch(e){message('Falha ao iniciar o ensaio')}}
