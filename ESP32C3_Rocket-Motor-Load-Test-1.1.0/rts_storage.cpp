                                    ...
                                    CampaignStatistics &statistics) const
{
    ...
    for (uint32_t i = 1; i < header.sampleCount; ++i)
    {
        ...
        impulse += ((double)previousPositive + (double)currentPositive) * 0.5 *
    ...
    statistics.impulseGramSeconds = (float)impulse;
    statistics.durationSeconds = (float)(lastUs - firstUs) / 1000000.0f;
