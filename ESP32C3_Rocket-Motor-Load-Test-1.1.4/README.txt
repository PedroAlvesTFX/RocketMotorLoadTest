...
Release 1.1.3
- Aquisição executada antes dos demais módulos no loop principal.
- Processamento do servidor Web suspenso durante STATE_RECORDING.
- Redução de interferências do HTTP/WiFi na coleta do HX711 a 80 SPS.
- Encerramento automático quando a leitura permanece <= 10 g por 250 ms.
- Leituras negativas também encerram o ensaio após o mesmo tempo de confirmação.
- Limite máximo de 30 segundos preservado.


Release 1.1.4
-------------
- Permite editar ou inserir a descrição de qualquer ensaio gravado.
- A edição pode ser feita no histórico pelo ícone de lápis.
- A página individual do ensaio também possui o botão Editar.
- A descrição permanece limitada a 50 caracteres.
- A atualização altera somente o cabeçalho do arquivo .rts, preservando todas as amostras.
- O CSV e as páginas Web passam a mostrar imediatamente a descrição atualizada.
- A alteração é permitida somente no estado IDLE.
