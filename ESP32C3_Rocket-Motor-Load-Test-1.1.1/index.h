...
th:nth-child(1),td:nth-child(1){width:90px;text-align:left}
th:nth-child(2),td:nth-child(2){width:300px;text-align:left;white-space:nowrap}
th:nth-child(3),td:nth-child(3){width:130px;text-align:right}
th:nth-child(4),td:nth-child(4){width:110px;text-align:right}
th:nth-child(5),td:nth-child(5){width:190px;text-align:center;white-space:nowrap}
...
async function startTest(){try{const r=await fetch('/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({datetime:browserDateTime()})});message(await r.text())}catch(e){message('Falha ao iniciar o ensaio')}}
...
</style></head><body><div class="wrap"><div class="top"><div><h1 id="title">Ensaio</h1><div id="datetime">Carregando...</div></div><div><a class="button" href="/">Voltar</a> <a id="csv" class="button" href="#">Baixar CSV</a> <button class="button" style="border:0;color:#fff;background:#8b1a1a;cursor:pointer" onclick="deleteCurrent()">Apagar</button></div></div>
